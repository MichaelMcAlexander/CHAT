...
//Rare exceptions to indicate a bug in the _practice code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_practice = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_practice]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.practice.module"
